...
| [`edb-append-system-prompt`](packages/edb-append-system-prompt) | `pi install npm:@agnishc/edb-append-system-prompt` | Set a per-session system prompt injection with enable/disable toggle |
...
| ~~[`edb-claude-proxy`](packages/edb-claude-proxy)~~ | ~~`pi install npm:@agnishc/edb-claude-proxy`~~ | ⚠️ DEPRECATED — `claude_proxy` tool, delegate tasks to Claude Code CLI (no longer maintained) |
...
| ~~[`edb-gemini-proxy`](packages/edb-gemini-proxy)~~ | ~~`pi install npm:@agnishc/edb-gemini-proxy`~~ | ⚠️ DEPRECATED — `gemini_proxy` tool, delegate tasks to Google Gemini CLI (no longer maintained) |
