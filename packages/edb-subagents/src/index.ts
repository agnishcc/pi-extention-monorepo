...
export default function (pi: ExtensionAPI) {
	...
	/** Build the full type list text dynamically — only includes enabled agents. */
...
- Choose the agent type that best fits the task based on the descriptions above.
			...
			parameters: Type.Object({
				...
				fallback_models: Type.Optional(
					Type.Array(Type.String(), {
						description:
							'Ordered list of fallback models (e.g. ["anthropic/claude-haiku-4-5", "openai/gpt-4o-mini"]) to try if the primary model fails with a provider error (rate limit, auth, timeout, overload). Ordinary task failures do not trigger fallback.',
					}),
				),
